...
— ключ, отправляемый Сервером аутентификации KDC Клиенту. Шифруется с
использованием хеша удостоверяющих данных (например, пароля) учетной
записи пользователя. Необходим для шифрования следующего сообщения Клиента
к KDC или Серверу Приложений.
...
1. Клиент аутентифицируется в KDC и в случае успеха получает TGT.
    1. Пользователь выполняет вход в систему на клиентском хосте (1),
       ...
       **принципал сервиса**, которые посылаются в виде `AS_REQ` (
       ...
       <br>`AS_REQ` содержит следующие данные в открытом виде:
        - User Principal Name (UPN) — имя **принципала пользователя**
        - Service Principal Name (SPN) — имя **принципала сервиса**,
    ...
    2. После проверки наличия **принципала пользователя** и **принципала
       ...
       **аутентифицированным**. После этого сведения об удостоверяющих
       ...
       <br>`AS_REP` содержит следующие зашифрованные данные:

        - зашифрованные с использованием удостоверяющих данных (хеша)
          **принципала пользователя** (**эти данные знают обе стороны**):
            - случайным образом сгенерированный уникальный сеансовый
              ключ (Copy of the Session Key);
            - время жизни этого ключа (Time To Live (TTL);
...
3. Клиент аутентифицируется на Сервере приложений, предъявляя
   действительный TGS (сервисное разрешение)
    1. Клиент (1) посылает (11) сообщение `AP_REQ` соответствующему
       ...
       <br>В этом сообщении
       содержится:
        - Сервисное разрешение (ST), полученное в предыдущем ответе
          (10), подтверждающее личность пользователя;
        - Структура **Authenticator** пользователя, как определено выше для
          сообщения `TGS_REQ`. Эта структура **Authenticator** зашифрована
          с помощью ключа сессии Сервиса приложений, полученная из
          предыдущего сообщения `TGS_REP` (10), и которая валидна в
          течение 5 минут.
...
- Клиент-серверная аутентификация
    - TLS аутентифицирует сервер по умолчанию
    - Kerberos аутентифицирует клиента по умолчанию
...
- [ktab](https://docs.oracle.com/javase/8/docs/technotes/tools/windows/ktab.html)
    ...
    - пары "принципал-ключ", перечисленные в таблице ключей, позволяют
      службам, работающим на хосте, аутентифицировать себя в центре
...
## System properties для аутентификации Kerberos

- java.security.krb5.conf — путь к krb5.conf файлу. Можно разместить файл
  krb5.conf в директории проекта
