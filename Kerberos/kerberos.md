...
— сетевой [протокол аутентификации][01], позволяющий передавать
...
1. Пароли или иные удостоверяющие данные никогда не пересылаются по сети.
   ...
   произойти подмена сообщения или другая атака.
2. Выдвигается обязательное требование, что информация о
   ...
   использует для
   входа/логина. После того, как произошёл первоначальный обмен в рамках
...
— структура данных, содержимое которой известно только издателю этого
разрешения, и какой-либо стороне или сторонам, к которым это разрешение
имеет отношение.
...
TGS также называют сервисным билетом, который используется для получения
доступа к требуемым доменным ресурсам. По умолчанию действителен в
течение 10 часов, не может быть обновлен, а должен каждый раз запрашиваться
повторно, в отличие от TGT.
...
1. Предварительная проверка подлинности — получение TGT
    ...
    2. После проверки наличия **принципала пользователя** и **принципала
       ...
       **аутентифицированным**. После этого сведения об удостоверяющих
       ...
       расшифровки более, чем на 5 минут (значение параметра `Time Skew` по
       умолчанию), то будет отправлен ответ `PreAuth failed`. При
       легитимной ситуации набор атрибутов может быть
       расшифрован только на контроллере домена.
       <br>
       ...
       <br>В самом `AS_REP` фигурируют атрибуты:
        - зашифрованные с использованием удостоверяющих данных
          **принципала пользователя** (хешем):
            ...
            - времени жизни этого ключа (Time To Live (TTL) — по умолчанию
              действителен в течение 10
              часов);
...
2. Финальная проверка подлинности — получение TGS
...
3. Авторизация клиента на Сервере приложений
    1. Клиент (1) посылает (11) сообщение `AP_REQ` соответствующему
       ...
       состоит из:
        ...
        - Клиент может запросить (с помощью флаг-поля в этом сообщении),
          что ему требуется провести **взаимную аутентификацию**. В этом случае
...
## Требования при разворачивании окружения

- Время на клиенте и сервере было синхронизировано, это необходимо для
  корректной работы Kerberos.
- Клиенты Kerberos ищут свои сервера по доменным именам, поэтому необходимо
  настроить DNS и убедиться, что имена серверов успешно разрешаются.
